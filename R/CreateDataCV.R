...
.CreateDataCV<- function(net_object               , p          = 0.75 , g           = 50, 
  ...
  appear[as.character(as.integer(stats$f_position))] <- 1
  deg                                    <- stats$final_deg[as.character(as.integer(stats$f_position))]
  if (FALSE == exclude_end) {
      if (net_type == "directed") {
          prob_em_each              <- matrix(0 , nrow = sum(unique_time > use_time) , ncol = length(stats$f_position))
          colnames(prob_em_each)    <- as.integer(stats$f_position)
          m_each                    <- rep(0 , sum(unique_time > use_time))
          deg_each                  <- matrix(0 , nrow = sum(unique_time > use_time) , ncol = length(stats$f_position))
          colnames(deg_each)        <- as.integer(stats$f_position)
          deg_each[1,]              <- deg
          time_each                 <- unique_time[unique_time > use_time]
          for (i in 1:length(time_each)){
              new_links                            <- in_node[time_stamp == time_each[i]]
              new_links                            <- new_links[appear[as.character(as.integer(new_links))] == 1]
              m_each[i]                            <- length(new_links)
              aaa                                  <- table(new_links)
              prob_em_each[i , as.character(as.integer(labels(aaa)[[1]]))]   <- aaa
              prob_em_each[i , as.character(as.integer(labels(aaa)[[1]]))]   <- prob_em_each[i, as.character(as.integer(labels(aaa)[[1]]))]/ m_each[i] 
          if (i < length(time_each)) {
              deg_each[i + 1,]                     <- deg_each[i,];    
              deg_each[i + 1,as.character(as.integer(labels(aaa)[[1]]))]     <- deg_each[i + 1 , as.character(as.integer(labels(aaa)[[1]]))] + aaa
          }
      }
        ...
        for (i in 1:length(time_each)){
            ...
            new_in_links      <- new_in_links[appear[as.character(as.integer(new_in_links))] == 1]
            new_out_links     <- out_node[time_stamp == time_each[i]]
            new_out_links     <- new_out_links[appear[as.character(as.integer(new_out_links))] == 1]
  ...
  #exclude_end = TRUE
      ...
      if (net_type == "directed") {
          ...
          for (i in 1:length(time_each)){
              ...
              new_links_final <- new_links[deg_each[i,as.character(as.integer(new_links))] < deg_max]
              m_each[i]       <- length(new_links_final)
              aaa             <- table(new_links)
              bbb             <- table(new_links_final)
              prob_em_each[i, as.character(as.integer(labels(bbb)[[1]]))] <- bbb
              prob_em_each[i, as.character(as.integer(labels(bbb)[[1]]))] <- prob_em_each[i, as.character(as.integer(labels(bbb)[[1]])) ]/ m_each[i] 
