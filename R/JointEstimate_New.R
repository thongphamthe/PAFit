JointEstimate_new <- function(raw_net, 
                          net_stat, 
                          stop.cond   = 10^-8 ,
                          mode_reg_A  = 0     , 
                          p           = 0.75  ,
  ...
  #print("start")
  data_cv   <- .CreateDataCV(raw_net, p = p, deg_thresh = 0)
  cv_result <- .performCV_new(data_cv, stop_cond = stop.cond, mode_reg_A = mode_reg_A, print_out = print.out)
  ...
  result_temp     <- PAFit_new(net_stat,
                           mode_f      = "Log_linear",
