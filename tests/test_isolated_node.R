...
for (ii in 1) {
    ...
    result_temp_temp <- joint_estimate(net_new, stats_new, stop_cond = 10^-3)
    ...
    result    <-  only_A_estimate(as.PAFit_net(net_approximate), net_stats, stop_cond = 10^-3)
    result_temp <- joint_estimate(as.PAFit_net(net_approximate), net_stats, stop_cond = 10^-3)
