...
for (bin in c("FALSE","TRUE")) {
  deg_thresh <- 0
  net_stats <- GetStatistics(net$graph,deg_threshold = deg_thresh, Binning = bin, G = 10, CompressMode = 2,
                             net_type = "undirected")
  net_stats <- GetStatistics(net$graph,deg_threshold = deg_thresh, Binning = bin, G = 10, CompressMode = 3, CustomTime = 0:20,
                             net_type = "directed") 
